...
print(
  ggplot(gd, aes(Year, Crude.Rate, colour = Age.Group)) + geom_line() + facet_wrap(facets = ~ Race) + xlab("Year") + ylab("Crude Rate of death") + ggtitle("All US Male Firearm Deaths by Age")
)
...
print(
  ggplot(gd_suicide, aes(Year, Crude.Rate, colour = Age.Group)) + geom_line() + facet_wrap(facets = ~ Race) + xlab("Year") + ylab("Crude Rate of death") + ggtitle("US Male Suicides using a Firearm by Age")
)
